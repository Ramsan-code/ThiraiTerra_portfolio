...
export function ThemeToggle() {
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="relative z-10 flex h-7 w-7 items-center justify-center rounded-full bg-foreground shadow-lg"
        initial={false}
        animate={{
          x: isDark ? 28 : 0,
        }}
        ...
        <AnimatePresence mode="wait" initial={false}>
          {isDark ? (
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1, rotate: 0 }}
              exit={{ opacity: 0, scale: 0.5, rotate: -45 }}
              className="h-3 w-3 rounded-full bg-background" // Circle for Light
            />
          )}
        </AnimatePresence>
      </motion.div>
